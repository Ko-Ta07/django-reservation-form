...
class MyPageView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        print(request.user)
        staff_data = Staff.objects.filter(user=request.user).select_related('user').select_related('store')
        staff_data = staff_data[0] if staff_data.exists() else None
        year = self.kwargs.get('year')
        month = self.kwargs.get('month')
        day = self.kwargs.get('day')
        start_date = date(year=year, month=month, day=day)
...
def Holiday(request, year, month, day, hour):
    staff_data = Staff.objects.get(user=request.user)#(id=request.user.id)
    start_time = make_aware(datetime(year=year, month=month, day=day, hour=hour))
    end_time = make_aware(datetime(year=year, month=month, day=day, hour=hour + 1))
